...
2. Открыть терминал, перейти в каталог с файлами ```MyExceptions/*.java```, ```TestTask.java```, ```TypeHandler.java``` командой ```cd <path>```, после чего cкомпилировать код утилитой ```javac```: ```javac TestTask.java```, а потом уже запустить утилитой ```java```: ```java TestTask [args]```